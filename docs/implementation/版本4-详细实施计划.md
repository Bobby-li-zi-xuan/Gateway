...
> 前置依赖：版本 1、2、3 已完成并跑通（见《版本3-详细实施计划.md》），本文档按 **V3 已合并** 为默认基线。
> 本文档的关键代码段是**实现参考**，允许按你的编码风格调整；但接口契约（配置结构、端点、错误体、指标名）请保持一致。
>
> ⚠️ 2026-08-10：当前工作区 V1 + V2 已落地（决策引擎 / 插件框架 / Scorer 信号动态调权 / BudgetFilter 占位均在），V3 按《版本3-详细实施计划.md》实施后合并。版本 4 的治理代码放在 `governance/` 包（与设计文档的模块边界对齐），决策侧的预算降级复用 V2 的“插件写信号 → 过滤器消费 → Scorer 动态调权”机制；`ChatGatewayService` 只做“接线”改造，不把治理逻辑堆进主流程类。
...
| 编号 | 手敲内容 | 位置 | 为什么必须手敲 |
|---|---|---|---|
| H1 | 令牌管理 `TokenManager`（创建随机令牌 / 哈希存储 / 校验时序 / 吊销） | 第 8 节 S5 | “凭证只能存哈希、明文只出现一次”是身份管理的核心心法 |
| H2 | 限流器 `RateLimiter`（环形时间桶滑动窗口 / 原子计数 / 多维度 / Token 占位修正） | 第 9 节 S6 | “管速度”的双层治理第一半，无锁读 + 原子写是并发点睛 |
| H3 | 预算管理器 `BudgetManager`（预检 → 扣减 → 结算 / 周期惰性重置 / CAS 防超扣 / 预警） | 第 10 节 S7 | “管总量”的双层治理第二半，**并发下不超扣**是版本 4 招牌 |
| H4 | Token 计量 `TokenMeter`（近似算法 / 单价表 / 流式增量累计 / 误差标注） | 第 11 节 S8 | “按字数收钱”的收银机，流式边出字边计费是难点 |
| H5 | 预算降级闭环 `BudgetFilter` 真实化（信号写入 / 逐候选淘汰 / 与 Scorer 动态调权对接） | 第 12 节 S9 | “花完钱自动换便宜餐厅”，信号驱动的自适应调度是本项目亮点 |
| H6 | 用量落库 `UsageLedger`（内存缓冲 / 定时批量写 H2 / 失败重试） | 第 13 节 S10 | “缓存 + 批量落库”避免每请求一次磁盘 IO，账本不拖慢收银 |
...
| `GatewayProperties` | health / decision / channels / models / policies / plugins（V3 已合并 execution 段与 channels[].timeout，第 5.1 节示例省略） | 仅新增 `tokens` 段（启动种子）；`governance` 段由独立的 `GovernanceProperties`（prefix `gateway.governance`）绑定 |
...
1. **流式计量回调**：V3 的 `StreamProxy` 逐 chunk 触发 `MeteringCallback`（onChunk / onFinish），V4 把空实现替换为真实累加（第 14.3 节）。
2. **降级链执行**：V3 的 `FallbackChainExecutor` / `StreamProxy` 接收候选链并执行。V4 的接线代码（第 14 节）在 V3 的 `ChatGatewayService` 签名上扩展。
...
│   │   └── budget/TokenUsageStore.java         （新增：令牌额度内存账本，见 12.2）
│   ├── meter/
...
├── infra/
│   ├── config/GatewayProperties.java           （改造：仅新增 tokens 段；governance 段由独立 GovernanceProperties 绑定）
...
governance/service/GovernanceServiceTest.java（接线：成功结算 / 失败不扣钱 / REJECT 预检拦截，见 20.1）
```
...
### 5.1 `model.yml`（V4 增量结构，基于 V3 合并后基线；示例省略 V3 段）
...
```yaml
# 示例省略 V3 的 execution 段与 channels[].timeout——V3 合并后追加，与本版本互不干扰，完整结构见《版本3-详细实施计划.md》5.1
gateway:
  health:                       # V1 保留
    intervalSeconds: 5
    timeoutMs: 2000
...
public class TokenDao {
     ...
     *  warmup 恢复 / findById / TokenUsageStore.restore 都靠本方法拿令牌，
...
统一语义：吊销 = 内存删除 + DB 标记 + warmup 恢复时过滤 revoked_at IS NOT NULL——
吊销后的令牌任何路径（缓存 / findByHash / warmup）都查不到，一律 UNKNOWN（401），重启前后一致
```
...
void validate_rejectsExpiredAndDisabledAndIp() {
    TokenDao dao = mock(TokenDao.class);          // revoke 依赖 dao.findAll() 按 id 查令牌
    ...
    //（mock 的 findByHash 返回 null 即该语义；重启后 warmup 同样过滤，前后一致，无 403 漂移）
...
3. 窗口和 + cost(本次消耗) ≤ limit → 放行并累加；否则拒绝。
```
...
public class RateLimiter {
    ...
    public boolean tryAcquire(RateLimitRule rule, long cost) {
        ...
        synchronized (bucket) {
            ...
            if (cost > bucket.limit) return false;  // 快速失败：窗口和 ≥ 0，cost 单独超限必然拒绝
...
- **`cost > limit` 快速失败**：窗口和 ≥ 0，单次 cost 超过窗口上限时求和判断必然为真——退避无用；放行会直接撑爆“窗口总和 ≤ limit”不变量。限流器只负责拒绝，不可重试的语义区分（413 `request_too_large`）由调用方 `GovernanceService` 完成（14.5）；
...
@Test
void singleCostExceedsLimit_alwaysRejected() {
    RateLimiter limiter = new RateLimiter();
    RateLimitRule rule = new RateLimitRule(RateLimitKey.token("tok-x"), 1000, 100, true);

    assertThat(limiter.tryAcquire(rule, 200)).isFalse();   // 单请求 200 > 上限 100：永远拒绝
    assertThat(limiter.windowCount(RateLimitKey.token("tok-x"), 1000)).isEqualTo(0);  // 拒绝不计数
}
...
private static ChatChunk chunk(String delta) {
    return new ChatChunk("id-1", "chat.completion.chunk", 0, "qwen",
            ...
            null, null);   // usage / error：V3 扩展的 7 字段，测试辅助不关心，置空
}
```
...
/**
 * 治理插件（阶段由 PluginEngine.runPhase 决定，本插件只实现 beforeDecision；order=100 最先执行）：
...
V3 计划中的 `MeteringCallback` 是无上下文空实现；V4 把它扩展为**带上下文的计量契约**（实施 V3 时按本签名落代码）：
...
`ChatGatewayService` 的 `complete` / `stream` 参数签名不变，仅返回类型从 `ChatCompletion` 改为 `ChatResult`（`stream` 为 void 不变），Controller 调用处改一行（`gatewayService.complete(...).completion()`）。
...
| `request_too_large` | 413 | 单请求预估用量超过限流窗口上限（不可重试，不带 Retry-After） | V4 |
...
10. **V3 契约对齐**：MeteringCallback / ChatResult 是 V4 对 V3 计划的两处契约扩展，实施 V3 时按本文档签名落代码（此时对齐成本最低，避免 V3 落地后返工）。
...
- 管理端点 `GovernanceAdminController` 会被 V5 的 Admin API（带管理密钥 + 审计日志）替换，**接口路径尽量保持一致**（`/v1/admin/*`）；V5 除加鉴权层与审计表外，还涉及三处语义调整：ChannelStore 的 DB 权重覆盖机制移除（配置权威）、渠道写操作改配置驱动、GatewayProperties/GovernanceProperties 变纯绑定类（详见《版本5-详细实施计划.md》2.1）；
