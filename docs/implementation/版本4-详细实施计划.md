...
| `RequestIdFilter` | Filter（ATTR=requestId） | GovernanceFilter 挂在其后（@Order(2)），先取链路 ID 再做鉴权 |
...
| `x-gateway-token-remaining` | GovernanceService 预检后写入 | 令牌剩余额度比率（信号备用） |
...
> `BudgetDao`（按主键 upsert 预算定义、更新/读取 used_value 快照）、`UsageDao`（`batchUpdate` 批量插入 + `SELECT token_id, SUM(token_in), SUM(token_out), SUM(cost) ... GROUP BY token_id` 汇总查询）、`ChannelDao`（渠道 CRUD + 软删除）结构同款，由 Codex 按上表字段实现，本节不重复贴码。
...
public class BudgetManager {
    ...
    /**
     * 预检：当前周期剩余是否 >= amount（不改变任何状态）。
     * @return true = 可放行
     */
    public boolean precheck(BudgetScope scope, String scopeValue, double amount) {
        Budget b = budgets.get(new BudgetKey(scope, scopeValue));
        if (b == null) return true;                 // 未配置预算 = 不限
        synchronized (b) {
            lazyResetIfNeeded(b);
            return b.toUnit(amount) <= b.toUnit(b.remaining());
        }
    }
     ...
     */
    public ConsumeResult consume(BudgetScope scope, String scopeValue, double amount) {
        Budget b = budgets.get(new BudgetKey(scope, scopeValue));
        if (b == null) return ConsumeResult.ok(1.0);   // 未配置 = 不扣
        synchronized (b) {
            lazyResetIfNeeded(b);
            long unit = b.toUnit(amount);
...
void concurrentConsume_neverExceedsLimit() throws InterruptedException {
    ...
    for (int i = 0; i < threads; i++) {
        futures.add(pool.submit(() -> {
            start.await();
            return mgr.consume(BudgetScope.GLOBAL, "", 10).ok();
        }));
    }
    start.countDown();
...
void periodRollover_resetsUsage() {
    ...
    mgr.consume(BudgetScope.GLOBAL, "", 60);
...
void precheck_doesNotDeduct() {
    BudgetManager mgr = new BudgetManager(mock(BudgetDao.class), List.of(), mock(GatewayMetrics.class));
    mgr.upsert(BudgetScope.GLOBAL, "", LimitType.TOKEN, BudgetPeriod.DAILY, 100);
    assertThat(mgr.precheck(BudgetScope.GLOBAL, "", 100)).isTrue();
    assertThat(mgr.precheck(BudgetScope.GLOBAL, "", 1)).isFalse();   // 预检不改状态
...
public class TokenMeter implements MeteringCallback {
    ...
    /** 已结算结果：requestId -> MeteredUsage（onFinish 写入，GovernanceService.takeResult 取走） */
    private final Map<String, MeteredUsage> finished = new ConcurrentHashMap<>();
    ...
    public MeteredUsage onFinish(MeteringContext ctx, ChatCompletion.Usage usage, boolean success) {
        ...
        if (tracker == null) {
            ...
            result = new MeteredUsage(in, 0, cost(ctx.instance(), in, 0), true);
        } else {
            if (usage != null && usage.totalTokens() > 0) {
                tracker.applyFinalUsage(usage);            // 真实 usage 覆盖累计值
            }
            result = tracker.result(ctx.instance());
            if (!success) {
                // 失败请求：仍留痕（result=failure），但不扣预算（成本归零）
                result = new MeteredUsage(result.tokenIn(), result.tokenOut(), 0, result.estimated());
            }
        }
    ...
    /** 流结束后取最终计量结果（从 finished 移除；无则按请求文本估算兜底） */
    public MeteredUsage takeResult(String requestId, ModelInstance fallback, ChatRequest request) {
        MeteredUsage result = finished.remove(requestId);
        if (result == null) {
            long in = estimateInput(request);
            result = new MeteredUsage(in, 0, cost(fallback, in, 0), true);
        }
        return result;
    }
 ...
 * - 已用量定期对账写回 api_token 表（used_value 快照，UsageLedger 顺带触发）。
 */
@Component
public class TokenUsageStore {
    ...
    private void restore() {
        for (Map<String, Object> row : usageDao.sumByTokenId()) {
            used.computeIfAbsent((String) row.get("token_id"), k -> new AtomicLong())
                    .addAndGet(((Number) row.get("total_unit")).longValue());
        }
    }
...
class BudgetFilterTest {
    ...
    void setUp() {
        ...
        filter = new BudgetFilter(store, tokenManager, testProps());
    }
...
public class GovernanceService {
    ...
    public void stream(ChatRequest request, String requestId, Map<String, String> metadata,
        ...
        try {
            ...
            // 流已正常结束：取流式计量结果（onFinish 已写入 finished 表）
            ModelInstance inst = registry.findByAlias(request.model()).get(0);
            MeteredUsage usage = meter.takeResult(requestId, inst, request);
            settle(token, new ChatResult(null, inst), usage, true, elapsedMs(start), requestId);
            tokenRateAdjust(token, usage, estIn);           // 实际用量修正
        } catch (Exception e) {
            ledger.record(failureRecord(request, requestId, token, elapsedMs(start)));
            throw e;
    ...
    private void settle(ApiToken token, ChatResult result, MeteredUsage usage,
        ...
        double amount = "TOKEN".equals(token.quotaType())
                ? usage.totalTokens() : usage.cost();
        usageStore.consume(token, amount);                     // 令牌额度
        if (result.instance() != null) {
            budgetManager.consume(BudgetScope.CHANNEL, result.instance().channelId(), amount);
            budgetManager.consume(BudgetScope.MODEL, result.instance().model(), amount);
            budgetManager.consume(BudgetScope.GLOBAL, "", amount);
        }
...
public interface MeteringCallback {
     ...
     * TokenMeter 会写入内部 finished 表供 GovernanceService.takeResult 取走；
...
// buildChunk(id, model, delta, isLast)：isLast 时附加 usage（与网关同口径估算）
...
GovernanceService.stream
  ...
  ├─ 流正常结束 → meter.takeResult(requestId) → 结算 + 留痕（同上）
