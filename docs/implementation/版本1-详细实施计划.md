# 版本 1：详细实施计划（含关键代码段）

> 配套文档：《版本1-统一接入与基础网关（学习版）》讲“是什么、为什么”，本文档讲“怎么做”。
> 技术基线：Java 21、Spring Boot 3.2、WebFlux、Maven 多模块（ai-gateway / mock-model-server）。
> 本文档的关键代码段是**实现参考**，允许按你的编码风格调整；但接口契约（配置结构、端点、错误体）请保持一致。

---

## 1. 目标与范围

版本 1 完成后，系统支持：

- `POST /v1/chat/completions`：非流式与流式（SSE）两种调用。
- `GET /v1/models`：列出已注册的模型别名。
- 配置驱动：`channels`（渠道）+ `models`（别名 → 候选），启动时完整校验。
- 加权随机路由 + 健康过滤 + 失败切换（最多尝试 N 个候选）。
- mock 服务可动态调延迟、故障率、健康状态。
- 全链路请求 ID、结构化日志、基础指标。

**非目标**：插件框架（V2）、重试退避/熔断/冷却（V3）、令牌/限流/计量（V4）、热更新（V5）。

---

## 2. 与现有代码的关系（先盘点，再动手）

| 现有文件 | 现状 | 处理方式 |
|---|---|---|
| `ModelConfig`（gateway.models） | 只有实例列表，无渠道/权重/别名 | 重构为 `GatewayProperties`（channels + models + health） |
| `ModelRegistry` | 实例 Map，按 ID 查询 | 改造为“别名 → 候选列表”，增加启动校验 |
| `HealthChecker` | 5 秒探活，UP/DEGRADED 两态 | 增加连续失败阈值与 `isHealthy` 快照查询 |
| `OpenAIConnector` | 非流式 `Mono<Map>` | 改为类型化 DTO + 新增流式方法 |
| `ChatCompletionController` | `Map` 出入参、无流式 | 改为类型化 DTO + 流式 + 统一错误 |
| `WebClientFactory` | 已有连接池与超时 | 保留，增加按渠道应用请求头的能力 |
| mock `ChatController` | 流式用了 `@PatchMapping`（不标准） | 改为 `POST + produces=TEXT_EVENT_STREAM` |
| mock `HealthController` | 永远返回 UP | 增加健康开关（Behavior API 控制） |

---

## 3. 工程结构与新增文件

```
ai-gateway/src/main/java/com/aigateway/
├── GatewayApplication.java                    （已有，保留）
├── api/
│   ├── controller/ChatCompletionController.java（重构）
│   ├── controller/ModelController.java         （新增：/v1/models）
│   ├── dto/ChatRequest.java                    （新增：record）
│   ├── dto/ChatCompletion.java                 （新增：record）
│   ├── dto/ChatChunk.java                      （新增：record）
│   └── error/GlobalExceptionHandler.java       （新增）
├── core/
│   ├── domain/model/Channel.java               （新增：record）
│   ├── domain/model/ModelInstance.java         （已有，扩展 weight/channelId）
│   ├── exception/GatewayException.java         （已有，扩展状态码）
│   └── service/ChatGatewayService.java         （新增：路由+转发主流程）
├── infra/
│   ├── config/GatewayProperties.java           （重构自 ModelConfig）
│   ├── config/SecretResolver.java              （新增：env: 解析）
│   ├── http/WebClientFactory.java              （已有，保留）
│   └── web/RequestIdFilter.java                （新增）
├── state/
│   ├── health/HealthChecker.java               （重构）
│   └── registry/ModelRegistry.java             （重构）
├── execution/
│   └── connector/OpenAIConnector.java          （重构）
└── observability/GatewayMetrics.java           （新增：Micrometer）

mock-model-server/src/main/java/com/aigateway/mock/
├── controller/ChatController.java              （重构：标准流式）
├── controller/BehaviorController.java          （扩展：health 开关）
└── controller/HealthController.java            （重构：尊重健康开关）
```

---

## 4. 依赖（pom.xml）

`ai-gateway/pom.xml` 在现有 `spring-boot-starter-webflux` 基础上增加（代码段 S1）：

```xml
<dependency>
    <groupId>org.springframework.boot</groupId>
    <artifactId>spring-boot-starter-actuator</artifactId>
</dependency>
<dependency>
    <groupId>io.micrometer</groupId>
    <artifactId>micrometer-registry-prometheus</artifactId>
</dependency>
<dependency>
    <groupId>io.projectreactor</groupId>
    <artifactId>reactor-test</artifactId>
    <scope>test</scope>
</dependency>
```

说明：actuator + prometheus 用于 `/actuator/prometheus` 指标端点（V1 用请求计数即可，V6 再丰富）。

---

## 5. 配置设计（关键代码段 S2）

### 5.1 `application.yaml`（保留 server 配置）

```yaml
server:
  port: 8080

spring:
  config:
    import:
      - classpath:model.yml

management:
  endpoints:
    web:
      exposure:
        include: health, prometheus
```

### 5.2 `model.yml`（核心：渠道 + 模型候选）

```yaml
gateway:
  health:
    intervalSeconds: 5
    timeoutMs: 2000
    consecutiveFailures: 3
  channels:
    - id: mock-a
      provider: openai-compatible
      baseUrl: http://localhost:8001
      credentialsRef: ""          # 空表示无需密钥（mock）；真实上游写 env:OPENAI_API_KEY
      weight: 10
    - id: mock-b
      provider: openai-compatible
      baseUrl: http://localhost:8002
      credentialsRef: ""
      weight: 5
  models:
    - alias: qwen
      candidates:
        - channelId: mock-a
          model: qwen-large        # 上游实际模型名
          weight: 8
          capability:
            contextLength: 128000
            multimodal: false
            toolCallSupport: true
        - channelId: mock-b
          model: qwen-large
          weight: 2
          capability:
            contextLength: 32000
            multimodal: false
            toolCallSupport: false
    - alias: deepseek-code
      candidates:
        - channelId: mock-b
          model: deepseek-code
          weight: 1
          capability:
            contextLength: 64000
            multimodal: false
            toolCallSupport: true
```

要点：

- `alias` = 客户端传的模型名（对应 LiteLLM 的 `model_name`）；
- `candidates` = 候选实例列表（对应 LiteLLM 的同名多条记录）；
- 权重是相对比例：alias `qwen` 下 mock-a 占 8/10，mock-b 占 2/10。

---

## 6. 领域模型（关键代码段 S3）

```java
// core/domain/model/Channel.java
public record Channel(String id, String provider, String baseUrl,
                      String credentialsRef, int weight) {
    public boolean needAuth() {
        return credentialsRef != null && !credentialsRef.isBlank();
    }
}

// core/domain/model/ModelInstance.java（扩展自现有类）
public record ModelInstance(
        String instanceId,       // channelId + ":" + model，唯一
        String alias,            // 对外模型名
        String channelId,
        String model,            // 上游模型名
        int weight,
        Capability capability
) {}

// api/dto/ChatRequest.java（只保留 V1 需要的字段）
public record ChatRequest(
        String model,
        List<Message> messages,
        Boolean stream,
        Double temperature,
        Integer maxTokens
) {
    public record Message(String role, String content) {}

    public boolean streaming() {
        return Boolean.TRUE.equals(stream);
    }
}

// api/dto/ChatCompletion.java（OpenAI 非流式响应）
public record ChatCompletion(
        String id,
        String object,
        long created,
        String model,
        List<Choice> choices,
        Usage usage
) {
    public record Choice(int index, Message message, String finishReason) {
        public record Message(String role, String content) {}
    }
    public record Usage(int promptTokens, int completionTokens, int totalTokens) {}
}

// api/dto/ChatChunk.java（OpenAI 流式分片）
public record ChatChunk(
        String id,
        String object,
        long created,
        String model,
        List<ChunkChoice> choices
) {
    public record ChunkChoice(int index, Delta delta, String finishReason) {
        public record Delta(String content) {}
    }
}
```

注意：Jackson 反序列化时字段名会自动把 `finishReason` 与 JSON 的 `finish_reason` 映射（Spring Boot 默认开启 `SNAKE_CASE` 宽松匹配）。如遇问题，可给字段加 `@JsonProperty("finish_reason")`。

---

## 7. 配置绑定与启动校验（关键代码段 S4、S5）

### 7.1 `GatewayProperties`（代码段 S4）

```java
@Configuration
@ConfigurationProperties(prefix = "gateway")
public class GatewayProperties {

    private Health health = new Health();
    private List<ChannelDef> channels = new ArrayList<>();
    private List<ModelDef> models = new ArrayList<>();

    public static class Health {
        private long intervalSeconds = 5;
        private long timeoutMs = 2000;
        private int consecutiveFailures = 3;
        // getter / setter 省略（Lombok @Data 可加在类上）
    }

    @Data
    public static class ChannelDef {
        private String id;
        private String provider;
        private String baseUrl;
        private String credentialsRef = "";
        private int weight = 1;
    }

    @Data
    public static class ModelDef {
        private String alias;
        private List<CandidateDef> candidates = new ArrayList<>();
    }

    @Data
    public static class CandidateDef {
        private String channelId;
        private String model;
        private int weight = 1;
        private Capability capability;
    }
    // 各字段 getter/setter 省略
}
```

### 7.2 密钥解析（代码段 S5a）

```java
@Component
public class SecretResolver {
    /** 支持 "env:XXX" 或空字符串；启动时解析，运行期只保存值 */
    public Optional<String> resolve(String credentialsRef) {
        if (credentialsRef == null || credentialsRef.isBlank()) {
            return Optional.empty();
        }
        if (credentialsRef.startsWith("env:")) {
            String var = credentialsRef.substring(4);
            return Optional.ofNullable(System.getenv(var));
        }
        throw new GatewayException(500, "invalid_config",
                "credentialsRef 只支持 env: 引用: " + credentialsRef);
    }
}
```

### 7.3 `ModelRegistry` 构建与校验（代码段 S5b）

```java
@Component
public class ModelRegistry {

    private final Map<String, List<ModelInstance>> byAlias = new ConcurrentHashMap<>();
    private final Map<String, Channel> channels = new ConcurrentHashMap<>();
    private final SecretResolver secretResolver;

    public ModelRegistry(GatewayProperties props, SecretResolver secretResolver) {
        this.secretResolver = secretResolver;
        init(props);
    }

    private void init(GatewayProperties props) {
        // 1) 渠道：解析密钥引用（缺失即启动失败）
        for (GatewayProperties.ChannelDef def : props.getChannels()) {
            secretResolver.resolve(def.getCredentialsRef());   // 只做校验
            channels.put(def.getId(), new Channel(
                    def.getId(), def.getProvider(), def.getBaseUrl(),
                    def.getCredentialsRef(), def.getWeight()));
        }
        // 2) 模型候选：校验渠道存在、权重合法
        for (GatewayProperties.ModelDef def : props.getModels()) {
            if (def.getAlias() == null || def.getAlias().isBlank()) {
                throw new GatewayException(500, "invalid_config", "models 中存在空 alias");
            }
            if (def.getCandidates().isEmpty()) {
                throw new GatewayException(500, "invalid_config",
                        "alias[" + def.getAlias() + "] 没有任何候选");
            }
            List<ModelInstance> instances = def.getCandidates().stream().map(c -> {
                Channel channel = channels.get(c.getChannelId());
                if (channel == null) {
                    throw new GatewayException(500, "invalid_config",
                            "alias[" + def.getAlias() + "] 引用了不存在的渠道: " + c.getChannelId());
                }
                if (c.getWeight() <= 0) {
                    throw new GatewayException(500, "invalid_config",
                            "alias[" + def.getAlias() + "] 存在非正权重");
                }
                return new ModelInstance(
                        c.getChannelId() + ":" + c.getModel(),
                        def.getAlias(), c.getChannelId(), c.getModel(),
                        c.getWeight(), c.getCapability());
            }).toList();
            byAlias.put(def.getAlias(), List.copyOf(instances));
        }
        if (byAlias.isEmpty()) {
            throw new GatewayException(500, "invalid_config", "未配置任何模型");
        }
    }

    public List<ModelInstance> findByAlias(String alias) {
        return byAlias.getOrDefault(alias, List.of());
    }

    public Optional<Channel> findChannel(String channelId) {
        return Optional.ofNullable(channels.get(channelId));
    }

    public List<String> aliases() {
        return List.copyOf(byAlias.keySet());
    }
}
```

---

## 8. 加权随机选择（关键代码段 S6）

这是 V1 的核心算法：权重累加区间 + 随机落点。

```java
package com.aigateway.core.service;

import com.aigateway.core.domain.model.ModelInstance;

import java.util.List;
import java.util.Optional;
import java.util.concurrent.ThreadLocalRandom;

/**
 * 加权随机选择器（不可变，构建后线程安全）。
 * 算法：把候选按权重在数轴上依次累加，随机数落在哪个区间就选谁。
 */
public final class WeightedRandomPicker {

    private final List<ModelInstance> candidates;
    private final int[] cumulative;   // cumulative[i] = 前 i+1 个候选的权重和
    private final int totalWeight;

    public WeightedRandomPicker(List<ModelInstance> candidates) {
        this.candidates = List.copyOf(candidates);
        this.cumulative = new int[candidates.size()];
        int sum = 0;
        for (int i = 0; i < candidates.size(); i++) {
            sum += candidates.get(i).weight();
            cumulative[i] = sum;
        }
        this.totalWeight = sum;
    }

    public boolean isEmpty() {
        return totalWeight <= 0;
    }

    public Optional<ModelInstance> pick() {
        if (isEmpty()) {
            return Optional.empty();
        }
        int r = ThreadLocalRandom.current().nextInt(totalWeight); // [0, totalWeight)
        for (int i = 0; i < cumulative.length; i++) {
            if (r < cumulative[i]) {
                return Optional.of(candidates.get(i));
            }
        }
        return Optional.of(candidates.get(candidates.size() - 1)); // 防御
    }
}
```

边界行为（写单测时逐条验证）：

- 权重 8:2 → 选中概率约 80% / 20%；
- 单个候选、权重任意 → 永远选中；
- 空列表 → `isEmpty()` 为 true；
- 候选顺序不影响正确性（只影响区间划分）。

---

## 9. 主流程：路由 + 转发 + 失败切换（关键代码段 S7）

```java
package com.aigateway.core.service;

@Service
public class ChatGatewayService {

    private final ModelRegistry registry;
    private final HealthChecker healthChecker;
    private final OpenAIConnector connector;
    private final GatewayMetrics metrics;

    /** 非流式：失败时换下一个候选，最多尝试 maxAttempts 个 */
    public Mono<ChatCompletion> complete(ChatRequest request, String requestId) {
        return attemptComplete(request, requestId, healthyCandidates(request.model()), 0);
    }

    private Mono<ChatCompletion> attemptComplete(ChatRequest request, String requestId,
                                                 List<ModelInstance> candidates, int attempt) {
        if (candidates.isEmpty()) {
            return Mono.error(new GatewayException(503, "no_available_model",
                    "模型[" + request.model() + "] 当前没有可用实例"));
        }
        ModelInstance chosen = new WeightedRandomPicker(candidates).pick()
                .orElseThrow(() -> new GatewayException(503, "no_available_model", "候选为空"));
        log(requestId, "路由选择", chosen.instanceId());

        return connector.complete(chosen, request)
                .doOnSuccess(r -> metrics.success(request.model(), chosen.instanceId()))
                .doOnError(e -> metrics.failure(request.model(), chosen.instanceId()))
                .onErrorResume(e -> {
                    log(requestId, "候选失败，尝试下一个", chosen.instanceId() + " - " + e.getMessage());
                    List<ModelInstance> rest = remaining(candidates, chosen);
                    if (attempt + 1 >= maxAttempts() || rest.isEmpty()) {
                        return Mono.error(new GatewayException(502, "upstream_failed",
                                "所有候选均失败: " + e.getMessage()));
                    }
                    return attemptComplete(request, requestId, rest, attempt + 1);
                });
    }

    /** 流式：与上面同构，只是返回 Flux；首字节前失败才切换候选 */
    public Flux<ChatChunk> stream(ChatRequest request, String requestId) {
        return attemptStream(request, requestId, healthyCandidates(request.model()), 0);
    }

    private Flux<ChatChunk> attemptStream(ChatRequest request, String requestId,
                                          List<ModelInstance> candidates, int attempt) {
        if (candidates.isEmpty()) {
            return Flux.error(new GatewayException(503, "no_available_model",
                    "模型[" + request.model() + "] 当前没有可用实例"));
        }
        ModelInstance chosen = new WeightedRandomPicker(candidates).pick()
                .orElseThrow(() -> new GatewayException(503, "no_available_model", "候选为空"));
        log(requestId, "路由选择(流式)", chosen.instanceId());

        // 只对“尚未开始输出”的错误做切换；已经开始输出则直接失败（见 V3 安全重试边界）
        return connector.stream(chosen, request)
                .doOnNext(chunk -> metrics.streamChunk(request.model(), chosen.instanceId()))
                .onErrorResume(e -> {
                    log(requestId, "流式候选失败(首字节前)，尝试下一个",
                            chosen.instanceId() + " - " + e.getMessage());
                    List<ModelInstance> rest = remaining(candidates, chosen);
                    if (attempt + 1 >= maxAttempts() || rest.isEmpty()) {
                        return Flux.error(new GatewayException(502, "upstream_failed",
                                "所有候选均失败: " + e.getMessage()));
                    }
                    return attemptStream(request, requestId, rest, attempt + 1);
                });
    }

    private List<ModelInstance> remaining(List<ModelInstance> candidates, ModelInstance chosen) {
        return candidates.stream()
                .filter(c -> !c.instanceId().equals(chosen.instanceId()))
                .toList();
    }

    private List<ModelInstance> healthyCandidates(String alias) {
        return registry.findByAlias(alias).stream()
                .filter(inst -> healthChecker.isHealthy(inst.channelId()))
                .toList();
    }

    private int maxAttempts() {
        return 2; // 可配置化（V5 热更新），V1 先用常量
    }
}
```

设计要点：

- 每次尝试用“剩余候选”重新做加权随机（而不是固定顺序），保证失败切换后权重仍然生效；
- 非流式切换 = 简单换候选（V1），V3 再引入重试退避/熔断；
- 流式目前也只处理“首字节前”的失败——这个边界从 V1 就写清楚，V3 直接沿用。

---

## 10. Connector：非流式 + 流式 SSE（关键代码段 S8）

```java
package com.aigateway.execution.connector;

@Component
public class OpenAIConnector {

    private final WebClient webClient;
    private final ModelRegistry registry;
    private final SecretResolver secretResolver;

    public OpenAIConnector(WebClient webClient,
                           ModelRegistry registry,
                           SecretResolver secretResolver) {
        this.webClient = webClient;
        this.registry = registry;
        this.secretResolver = secretResolver;
    }

    public Mono<ChatCompletion> complete(ModelInstance instance, ChatRequest request) {
        return webClient.post()
                .uri(channelBaseUrl(instance) + "/v1/chat/completions")
                .headers(h -> applyAuth(h, instance))
                .bodyValue(request)                 // stream=false 或 null
                .retrieve()
                .onStatus(HttpStatusCode::isError,
                        resp -> resp.bodyToMono(String.class)
                                .map(body -> new GatewayException(502, "upstream_error",
                                        "上游返回 " + resp.statusCode() + ": " + body)))
                .bodyToMono(ChatCompletion.class);
    }

    public Flux<ChatChunk> stream(ModelInstance instance, ChatRequest request) {
        return webClient.post()
                .uri(channelBaseUrl(instance) + "/v1/chat/completions")
                .headers(h -> applyAuth(h, instance))
                .accept(MediaType.TEXT_EVENT_STREAM)
                .bodyValue(new ChatRequest(request.model(), request.messages(),
                        true, request.temperature(), request.maxTokens()))
                .retrieve()
                .bodyToFlux(String.class)           // 逐行收原始 SSE 文本
                .flatMap(line -> parseSseLine(line, request.model()));
    }

    private void applyAuth(HttpHeaders headers, ModelInstance instance) {
        registry.findChannel(instance.channelId())
                .filter(Channel::needAuth)
                .ifPresent(ch -> headers.setBearerAuth(
                        secretResolver.resolve(ch.credentialsRef()).orElse("")));
    }

    /** 解析 "data: {...}" / "data: [DONE]"；非 data 行（注释/空行）忽略 */
    private String channelBaseUrl(ModelInstance instance) {
        return registry.findChannel(instance.channelId())
                .map(Channel::baseUrl)
                .orElseThrow(() -> new GatewayException(500, "invalid_config",
                        "渠道不存在: " + instance.channelId()));
    }

    private Flux<ChatChunk> parseSseLine(String line, String model) {
        String trimmed = line.trim();
        if (!trimmed.startsWith("data:")) {
            return Flux.empty();
        }
        String payload = trimmed.substring(5).trim();
        if ("[DONE]".equals(payload)) {
            return Flux.empty();
        }
        try {
            ChatChunk chunk = objectMapper.readValue(payload, ChatChunk.class);
            return Flux.just(chunk);
        } catch (JsonProcessingException e) {
            return Flux.error(new GatewayException(502, "bad_upstream_sse",
                    "上游返回了无法解析的 SSE 数据: " + payload));
        }
    }
}
```

说明：

- `bodyToFlux(String.class)` 拿到的是 SSE 按行切好的文本（WebClient 已按 `\n\n` 分帧），逐行解析 `data:` 前缀即可；
- `flatMap` 返回 `Flux` 是为了能表达“这一行解析失败 → 中断流”；
- 真实上游若带鉴权头，`applyAuth` 会从渠道配置的 `env:` 引用取密钥。

---

## 11. 对外 API：Controller 与统一错误（关键代码段 S9、S10）

### 11.1 `ChatCompletionController`（代码段 S9）

```java
@RestController
public class ChatCompletionController {

    private final ChatGatewayService gatewayService;

    @PostMapping("/v1/chat/completions")
    public Mono<ResponseEntity<?>> chat(@RequestBody ChatRequest request,
                                        ServerWebExchange exchange) {
        String requestId = requestId(exchange);
        if (request.streaming()) {
            Flux<ServerSentEvent<ChatChunk>> sse =
                    gatewayService.stream(request, requestId)
                            .map(chunk -> ServerSentEvent.builder(chunk).build());
            return Mono.just(ResponseEntity.ok()
                    .contentType(MediaType.TEXT_EVENT_STREAM)
                    .body(sse));
        }
        return gatewayService.complete(request, requestId)
                .map(ResponseEntity::ok);
    }

    @GetMapping("/v1/models")
    public Mono<ModelsResponse> models() {
        return Mono.just(new ModelsResponse(registry.aliases().stream()
                .map(a -> new ModelsResponse.Model(a))
                .toList()));
    }

    private String requestId(ServerWebExchange exchange) {
        return exchange.getAttributeOrDefault(RequestIdFilter.ATTR, UUID.randomUUID().toString());
    }
}
```

### 11.2 统一错误（代码段 S10）

```java
// core/exception/GatewayException.java（扩展）
public class GatewayException extends RuntimeException {
    private final int status;      // HTTP 状态码
    private final String type;     // OpenAI 风格错误类型

    public GatewayException(int status, String type, String message) {
        super(message);
        this.status = status;
        this.type = type;
    }
    // getter 省略
}

// api/error/GlobalExceptionHandler.java
@RestControllerAdvice
public class GlobalExceptionHandler {

    public record ApiError(String type, String message) {}

    @ExceptionHandler(GatewayException.class)
    public ResponseEntity<ApiError> handleGateway(GatewayException e) {
        return ResponseEntity.status(e.getStatus())
                .body(new ApiError(e.getType(), e.getMessage()));
    }

    @ExceptionHandler(WebExchangeBindException.class)
    public ResponseEntity<ApiError> handleBadRequest(WebExchangeBindException e) {
        return ResponseEntity.badRequest()
                .body(new ApiError("invalid_request", "请求参数不合法: " + e.getMessage()));
    }

    @ExceptionHandler(Throwable.class)
    public ResponseEntity<ApiError> handleOther(Throwable e) {
        return ResponseEntity.status(500)
                .body(new ApiError("internal_error", "网关内部错误"));
    }
}
```

错误码约定（V1 固定）：

| 场景 | HTTP | type |
|---|---|---|
| 请求体不合法 | 400 | `invalid_request` |
| 模型别名不存在/无可用实例 | 503 | `no_available_model` |
| 所有上游候选失败 | 502 | `upstream_failed` |
| 上游返回错误 | 502 | `upstream_error` |
| 内部错误 | 500 | `internal_error` |

---

## 12. 请求 ID 与指标（关键代码段 S11、S12）

### 12.1 `RequestIdFilter`（代码段 S11）

```java
@Component
public class RequestIdFilter implements WebFilter {
    public static final String ATTR = "requestId";

    @Override
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        String requestId = UUID.randomUUID().toString();
        exchange.getAttributes().put(ATTR, requestId);
        ServerWebExchange mutated = exchange.mutate()
                .request(builder -> builder.header("X-Request-Id", requestId))
                .build();
        return chain.filter(mutated);
    }
}
```

日志约定：GatewayService 中统一用 `log(requestId, event, detail)` 打印 `[requestId=xxx] event=xxx detail=xxx`，方便按 requestId 串起一次调用的完整链路。

### 12.2 `GatewayMetrics`（代码段 S12）

```java
@Component
public class GatewayMetrics {
    private final MeterRegistry meterRegistry;

    public GatewayMetrics(MeterRegistry meterRegistry) {
        this.meterRegistry = meterRegistry;
    }

    public void success(String alias, String instanceId) {
        counter("gateway.requests.total", alias, instanceId, "success").increment();
    }

    public void failure(String alias, String instanceId) {
        counter("gateway.requests.total", alias, instanceId, "failure").increment();
    }

    public void streamChunk(String alias, String instanceId) {
        counter("gateway.stream.chunks", alias, instanceId, "").increment();
    }

    private Counter counter(String name, String alias, String instance, String result) {
        return Counter.builder(name)
                .tag("model", alias)
                .tag("instance", instance)
                .tag("result", result)
                .register(meterRegistry);
    }
}
```

访问：`GET /actuator/prometheus`，可以看到 `gateway_requests_total{model="qwen",instance="mock-a:qwen-large",result="success"}` 这类指标。

---

## 13. 健康检查改进（关键代码段 S13）

在现有 `HealthChecker` 基础上增加“连续失败阈值 + 快照查询”：

```java
@Component
public class HealthChecker {
    private final GatewayProperties props;
    private final WebClient webClient;

    private final Map<String, Integer> consecutiveFailures = new ConcurrentHashMap<>();
    private final Map<String, Boolean> healthy = new ConcurrentHashMap<>();  // 最新快照

    public void start() {  // @PostConstruct 或 SmartLifecycle 启动定时任务
        Executors.newSingleThreadScheduledExecutor().scheduleAtFixedRate(
                this::checkAll, 0, props.getHealth().getIntervalSeconds(), TimeUnit.SECONDS);
    }

    private void checkAll() {
        registry.findAll().stream()
                .map(ModelInstance::channelId).distinct()
                .forEach(this::checkOne);
    }

    private void checkOne(String channelId) {
        registry.findChannel(channelId).ifPresent(ch ->
                webClient.get()
                        .uri(ch.baseUrl() + "/health")
                        .retrieve()
                        .bodyToMono(Map.class)
                        .timeout(Duration.ofMillis(props.getHealth().getTimeoutMs()))
                        .subscribe(
                                body -> mark(channelId, "UP".equals(body.get("status"))),
                                err -> mark(channelId, false)
                        ));
    }

    private void mark(String channelId, boolean up) {
        if (up) {
            consecutiveFailures.put(channelId, 0);
            healthy.put(channelId, true);
        } else {
            int fails = consecutiveFailures.merge(channelId, 1, Integer::sum);
            if (fails >= props.getHealth().getConsecutiveFailures()) {
                healthy.put(channelId, false);   // 连续失败达阈值 → 不健康
            }
        }
    }

    /** 路由调用：读快照，不做实时探测 */
    public boolean isHealthy(String channelId) {
        return healthy.getOrDefault(channelId, true);  // 初次探活前默认放行
    }
}
```

注意：

- 健康状态按**渠道**维护（V1 粒度），路由按渠道过滤；
- `isHealthy` 是纯内存读，请求路径零阻塞；
- mock 的 `/health` 返回 `status: DOWN` 时，连续 3 个周期后该渠道被剔除。

---

## 14. mock 服务端修改（关键代码段 S14）

### 14.1 流式接口改为标准 `POST + SSE`

```java
@PostMapping(value = "/v1/chat/completions", params = "stream=true",
             produces = MediaType.TEXT_EVENT_STREAM_VALUE)
public Flux<ServerSentEvent<Map<String, Object>>> chatStream(
        @RequestBody Map<String, Object> request) {
    ModelProfile profile = registry.getProfile();
    String content = gen.generate(profile);
    if (gen.shouldFail(profile)) {
        return Flux.error(new RuntimeException("Mock model internal error"));
    }
    String[] tokens = splitIntoTokens(content, profile.responseStyle());
    String id = "chatcmpl-" + UUID.randomUUID().toString().substring(0, 8);
    return Flux.fromArray(tokens)
            .delayElements(Duration.ofMillis((long) profile.streamingDelayMs()))
            .map(token -> ServerSentEvent.builder(buildChunk(id, profile.name(), token, false)).build())
            .concatWith(Mono.just(ServerSentEvent.builder(
                    buildChunk(id, profile.name(), "", true)).build()));
}
```

要点：删掉原来的 `@PatchMapping`；`produces=TEXT_EVENT_STREAM` 让 Spring 自动按 `data:` 格式输出，网关端 `bodyToFlux(String.class)` 就能逐行收到。

### 14.2 健康开关（BehaviorController 增加 health 参数）

```java
if (params.containsKey("health")) {
    registry.setHealthy(Boolean.parseBoolean(params.get("health").toString()));
}
```

`HealthController` 改为：

```java
return Map.of(
    "status", registry.isHealthy() ? "UP" : "DOWN",
    "activeRequests", registry.getCurrentRequests(),
    "queueSize", p.queueSize()
);
```

这样演示时可以直接 `POST /mock/behavior {"health": false}` 把某实例置为不健康，观察健康检查剔除。

---

## 15. 一次请求的完整时序（非流式）

```
Client
  │ POST /v1/chat/completions {model:"qwen", messages:[...]}
  ▼
RequestIdFilter（生成 requestId，注入 X-Request-Id）
  ▼
ChatCompletionController
  ▼
ChatGatewayService.complete()
  ├─ ModelRegistry.findByAlias("qwen") → [mock-a 权重8, mock-b 权重2]
  ├─ 过滤：healthChecker.isHealthy(channelId)（mock-b 若 DOWN 则剔除）
  ├─ WeightedRandomPicker.pick() → 选中 mock-a（约 80%）
  ▼
OpenAIConnector.complete()
  └─ WebClient POST http://localhost:8001/v1/chat/completions
        ▼
     mock ChatController（模拟延迟/故障/生成内容）
  ▼
成功 → ChatCompletion 返回客户端；失败 → 换 mock-b 再试一次
  ▼
GatewayMetrics 计数 → 结构化日志（requestId 贯穿）
```

流式时序相同，区别是 Connector 返回 `Flux<ChatChunk>`，Controller 以 SSE 分片转发。

---

## 16. 测试计划

### 16.1 单元测试（JUnit + Mockito / StepVerifier）

| 测试类 | 用例 | 断言要点 |
|---|---|---|
| `WeightedRandomPickerTest` | 权重 8:2 跑 1000 次 | 分布落在 75%~85% 之间（宽松断言避免 flaky） |
| `WeightedRandomPickerTest` | 单个候选 | 永远返回该候选 |
| `WeightedRandomPickerTest` | 空列表 | `isEmpty()` 为 true，`pick()` 为空 |
| `ModelRegistryTest` | 引用不存在渠道 | 启动抛 `GatewayException(invalid_config)` |
| `ModelRegistryTest` | 空候选/重复别名 | 启动失败，错误信息可读 |
| `ChatGatewayServiceTest` | 第一候选失败 → 第二候选成功 | 最终响应来自第二候选，指标记录一次失败一次成功 |
| `ChatGatewayServiceTest` | 全部候选失败 | 返回 502 `upstream_failed` |
| `ChatGatewayServiceTest` | 候选全被健康过滤 | 返回 503 `no_available_model` |
| `OpenAIConnectorTest` | SSE 文本解析 | `data: {...}` 解析为 ChatChunk；`[DONE]` 结束；垃圾行忽略 |
| `RequestIdFilterTest` | 两次请求 | 每个请求有独立 requestId |

关键测试示例（代码段 S15）：

```java
class WeightedRandomPickerTest {
    @Test
    void weightedDistribution_shouldFollowWeights() {
        var a = instance("a", 8);
        var b = instance("b", 2);
        var picker = new WeightedRandomPicker(List.of(a, b));

        int countA = 0;
        for (int i = 0; i < 1000; i++) {
            if (picker.pick().orElseThrow().instanceId().equals("a")) countA++;
        }
        assertThat(countA).isBetween(750, 850);   // 8/10 概率，宽松区间
    }

    @Test
    void emptyPicker_shouldReturnEmpty() {
        var picker = new WeightedRandomPicker(List.of());
        assertThat(picker.isEmpty()).isTrue();
        assertThat(picker.pick()).isEmpty();
    }
}
```

### 16.2 集成测试

`ChatGatewayIntegrationTest`：启动真实网关（随机端口）+ 两个 mock 实例，用 `WebTestClient` 验证：

1. 非流式返回 OpenAI 格式；
2. `stream=true` 返回 SSE 且最后一个 chunk 的 `finish_reason=stop`；
3. 把 mock-a 故障率调 100% 后，请求仍成功（降级到 mock-b）；
4. 把 mock-b 健康置 DOWN 后，`/v1/models` 仍返回但路由只走 mock-a。

---

## 17. 演示与验收（curl 命令）

```bash
# 1) 启动：mock 两个实例 + 网关
cd mock-model-server && mvn spring-boot:run -Dspring-boot.run.arguments=--server.port=8001 &
cd mock-model-server && mvn spring-boot:run -Dspring-boot.run.arguments=--server.port=8002 &
cd ai-gateway && mvn spring-boot:run

# 2) 非流式
curl -s http://localhost:8080/v1/chat/completions \
  -H 'Content-Type: application/json' \
  -d '{"model":"qwen","messages":[{"role":"user","content":"你好"}]}'

# 3) 流式
curl -N -s http://localhost:8080/v1/chat/completions \
  -H 'Content-Type: application/json' \
  -d '{"model":"qwen","messages":[{"role":"user","content":"你好"}],"stream":true}'

# 4) 故障注入：mock-a 100% 失败 → 观察自动切换
curl -s -X POST http://localhost:8001/mock/behavior \
  -H 'Content-Type: application/json' -d '{"errorRate":1.0}'

# 5) 健康剔除：mock-b 置为不健康 → 等一个检查周期后流量只走 mock-a
curl -s -X POST http://localhost:8002/mock/behavior \
  -H 'Content-Type: application/json' -d '{"health":false}'

# 6) 指标
curl -s http://localhost:8080/actuator/prometheus | grep gateway_
```

验收标准对照（与学习版文档一致）：

1. 一条命令启动全部服务；
2. 非流式/流式均返回正确 OpenAI 格式；
3. 加权比例接近配置（10% 以内波动）；
4. 故障切换、健康剔除可稳定复现；
5. 核心算法（加权随机/健康过滤/失败切换）有单测覆盖。

---

## 18. 任务清单（按顺序执行）

| # | 任务 | 产出 | 验收 |
|---|---|---|---|
| T1 | 加依赖、建 DTO 与领域模型 | `ChatRequest/ChatCompletion/ChatChunk/Channel/ModelInstance` | 编译通过 |
| T2 | 配置绑定与校验 | `GatewayProperties` + `SecretResolver` + `ModelRegistry` 重构 | 非法配置启动失败且报错可读 |
| T3 | 加权随机 | `WeightedRandomPicker` + 单测 | 边界用例全绿 |
| T4 | Connector 重构 | 类型化非流式 + 流式 SSE 解析 + 单测 | mock 直接调用两种模式均正常 |
| T5 | 主流程 | `ChatGatewayService`（路由+切换）+ 单测 | 故障切换/503/502 用例通过 |
| T6 | Controller 与错误处理 | 两个端点 + `GlobalExceptionHandler` | curl 演示通过 |
| T7 | 请求 ID 与日志 | `RequestIdFilter` + 日志约定 | 日志可按 requestId 串联 |
| T8 | 指标 | `GatewayMetrics` + actuator | `/actuator/prometheus` 有指标 |
| T9 | 健康检查改进 | 阈值 + 快照 + 单测 | 置 DOWN 后 3 个周期被剔除 |
| T10 | mock 重构 | 标准流式 + health 开关 | 网关流式/健康演示通过 |
| T11 | 集成测试 | `ChatGatewayIntegrationTest` | 4 个场景全过 |
| T12 | 文档与演示 | 更新 README / 录演示命令 | 按第 17 节可完整走一遍 |

---

## 19. 风险与注意点

1. **SSE 帧格式**：Spring 的 `ServerSentEvent` 输出与上游 `data:` 行解析都要先跑通再写业务逻辑；建议 T4 先做“网关 → mock 流式”的最小联调。
2. **WebClient 流式背压**：`bodyToFlux(String.class)` 默认按行缓冲，没问题；不要 `collectList()` 后再转发（会等全部结束，失去流式意义）。
3. **健康检查并发**：`consecutiveFailures` 用 `ConcurrentHashMap`，探活是异步 subscribe，不要在 `subscribe` 里做阻塞操作。
4. **Map 出入参的坑**：现有代码用 `Map` 透传，重构为 DTO 后要处理未知字段（`@JsonIgnoreProperties(ignoreUnknown = true)` 对 record 默认忽略未知字段，无需额外配置）。
5. **mock 多实例**：两个 mock 用不同端口启动，但共享同一份代码；确认 `ModelRegistry`（mock 侧）按启动参数选择 `ModelProfile`，避免两个实例配置互相覆盖。

